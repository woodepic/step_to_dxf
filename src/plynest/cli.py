...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--keepout-layer", action="store_true",
                   help="include the edge keep-out rectangle in the DXF")
...
def main(argv: list[str] | None = None) -> int:
    ...
    settings = RunSettings(
        ...
        export=ExportSettings(
            ...
            include_keepout=args.keepout_layer,
            engrave_labels_in_step=not args.no_engrave_in_step,
            engrave_tool_mm=to_mm(args.engrave_tool, unit),
        ),
    )
    ...
    if settings.export.mode == "step_per_sheet":
        paths = export_step(
            ...
            labels=job.labels, label_depth_mm=settings.labels.depth_mm,
            sheet_namer=lambda sh: sheet_name(sh.index, sh.thickness, unit),
        )
    else:
        paths = export(
            job.result, settings.export, args.out,
            labels=job.labels, label_depth_mm=settings.labels.depth_mm,
            parts=job.parts,
        )
